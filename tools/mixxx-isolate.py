    ...
    while True:
        try:
            ...
            if helper.returncode == 127:
                # user aborted password request, nothing happened so its
                # save to exit
            ...
            if line.startswith("STATUS:"):
                ...
                if status == "ISOLATION":
                    ...
                    mixxxProc = subprocess.Popen(
                        [args.mixxx, "--engine-cpu-set", args.cpuset_engine]
                        + args.mixxx_args
                    )
                    mixxxProc.wait()
...
def main():
    ...
    This program will separate one CPU core from the system and place the
