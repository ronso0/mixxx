...
WOverview::WOverview(
    ...
    m_pEndOfTrackBlinkTimer = make_parented<ControlProxy>("[App]", "indicator_500ms", this);
...
void WOverview::onEndOfTrackChange(double v) {
    ...
    m_endOfTrack = v > 0.0;
    // Note that this may make the eot indicator kick in with a first blink period
...
void WOverview::onEndOfTrackBlinkTimerChange(bool v) {
    if (!m_endOfTrack) {
        return;
    }
