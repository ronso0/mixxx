    ...
    if (m_isMainDeck && m_isComment) {
        ...
        connect(pEditCommentAction.get(),
                ...
                [this]() {
                    ...
                    // If there are more, all editors will open?
                    // This is unlikely to happen synchronously, so as soon as
                    // one editor gets focus, others will receive a focusOut event
                    // and close (and commit, hmm..)
                    if (isVisible()) {
