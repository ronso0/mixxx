...
void LoopingControl::setLoopOutToCurrentPosition() {
    ...
    if (m_pQuantizeEnabled->toBool() && pBeats) {
        ...
        if (pBeats->findPrevNextBeats(position, &prevBeatPosition, &nextBeatPosition, false)) {
            ...
            // Note: with quantize and playpos after an existing, inactive loop
            // the new loop_out might snap to the exact the same position as before.
            // Then LoopInfo would be the same and process() would not seek back
            // inside the loop, so we would (re)created and activated a loop
            // we'd never reach (when playing forward).
            // Let's invalidate the old loop end so adjustedPositionInsideAdjustedLoop()
            // returns a position inside the new/old loop.
            if (quantizedBeatPosition == m_oldLoopInfo.endPosition) {
