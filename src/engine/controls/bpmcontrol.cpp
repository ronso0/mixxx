...
double BpmControl::calcSyncAdjustment(bool userTweakingSync) {
    ...
    if (traceEnabled && adjustment - 1.0 > 0.01) {
        kLogger.trace() << "calcSyncAdjustment" << getGroup() << "***************";
...
double BpmControl::getBeatDistance(mixxx::audio::FramePos thisPosition) const {
    ...
    // sync against itself.
    if (traceEnabled) {
        kLogger.trace() << "getBeatDistance" << getGroup() << "thisPosition:"
                        << thisPosition;
    }
    ...
    if (traceEnabled) {
        kLogger.trace() << "getBeatDistance" << getGroup() << "dBeatPercentage:"
...
mixxx::audio::FramePos BpmControl::getNearestPositionInPhase(
    ...
    if (!thisPrevBeatPosition.isValid() || !thisNextBeatPosition.isValid() ||
        ...
        if (traceEnabled) {
            kLogger.trace() << "getNearestPositionInPhase" << getGroup() << "out of date"
...
mixxx::audio::FramePos BpmControl::getBeatMatchPosition(
    ...
    if (traceEnabled) {
        if (pOtherEngineBuffer) {
            kLogger.trace() << "getBeatMatchPosition" << getGroup() << "sync target:"
                            << pOtherEngineBuffer->getGroup();
        } else {
            kLogger.trace() << "getBeatMatchPosition" << getGroup() << "no sync target found";
        }
    }
    if (playing) {
        if (!pOtherEngineBuffer || pOtherEngineBuffer->getSpeed() == 0.0) {
...
void BpmControl::slotUpdateEngineBpm(double rateRatio) {
    ...
    if (traceEnabled) {
        kLogger.trace() << "slotUpdateEngineBpm" << getGroup() << "local BPM:"
                        << m_pLocalBpm->get()
                        << "rate ratio:"
                        << rateRatio;
        // This can be used to detect pitch shift issues with cloned decks
...
void BpmControl::slotUpdateRateSlider(double value) {
    if (traceEnabled) {
        kLogger.trace() << "slotUpdateRateSlider" << getGroup() << value;
    }
