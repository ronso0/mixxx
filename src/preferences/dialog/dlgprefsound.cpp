...
DlgPrefSound::DlgPrefSound(QWidget* pParent,
    ...
    // Store tr strings of labels where we will append (?) links
    m_apiLabelText = apiLabel->text();
    m_micMonitorModeLabelText = micMonitorModeLabel->text();
    m_micLatencyLabelText = latencyCompensationLabel->text();
    m_deckBusHintSText = deckBusHint->text();
...
void DlgPrefSound::updateColoredLinkTexts() {
    ...
    apiLabel->setText(m_apiLabelText + QChar(' ') +
            coloredLinkString(
                    m_pLinkColor,
                    QStringLiteral("(?)"),
                    MIXXX_MANUAL_SOUND_API_URL));

    micMonitorModeLabel->setText(m_micMonitorModeLabelText + QChar(' ') +
            coloredLinkString(
                    m_pLinkColor,
                    QStringLiteral("(?)"),
                    MIXXX_MANUAL_MIC_MONITOR_MODES_URL));

    latencyCompensationLabel->setText(m_micLatencyLabelText + QChar(' ') +
            coloredLinkString(
                    m_pLinkColor,
                    QStringLiteral("(?)"),
                    MIXXX_MANUAL_MIC_LATENCY_URL));
    ...
    QString deckBusHintStr = m_deckBusHintSText;
    deckBusHintStr += " " +
            coloredLinkString(m_pLinkColor,
                    tr("Find details in the Mixxx user manual"),
                    MIXXX_MANUAL_OUTPUT_AND_INPUT_DEVICES);
